...
const unenrollBrook = (arr) => {
  const result = arr.map(courseRoster =>
    courseRoster.filter(name => !name.includes('Brook'))
  );
...
const sortByDay = (arr) => {
  ...
  const eventsByDay = new Array(7).fill().map(() => []);

  
  arr.forEach(event => {
    
    const eventDays = daysOfWeek.filter(day => event.includes(day));
