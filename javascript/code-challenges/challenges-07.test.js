...
const removeThree = (idx, arr) => {
  if (index >= 0 && index + 2 < array.length) {
    ...
    console.error('Invalid index or not enough elements to remove.');
  }
  return array;
}
...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i < recipe.ingredients.length; i++) {
    ...
    const startIdx = ingredient.indexOf(' ') + 1; // Find the index after the first space
    const endIdx = ingredient.indexOf(' ', startIdx); // Find the index of the second space
    const foodName = ingredient.slice(startIdx, endIdx !== -1 ? endIdx : undefined);
...
const splitFoods = (recipe) => {
  ...
  const recipeText = trimmedLines.join(' ');

  
  const foodItems = recipeText.split(' and ');
