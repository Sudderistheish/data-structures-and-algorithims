...
function transformToLis(obj){const lis = [];
...
const count = (target, input) => {
  const totalCount = input.reduce((acc, arr) => {
    const occurrences = arr.reduce((count, num) => count + (num === target ? 1 : 0), 0);
...
const totalSum = (input) => {
  const sum = input.reduce((acc, arr) => {
    const arrSum = arr.reduce((partialSum, num) => partialSum + num, 0);
    return acc + arrSum;
...
const divisibleByFiveTwoToThePower = (input) => { 
  ...
  const powerOfTwo = (num) => Math.pow(2, num);
  ...
  for (const subArray of input) {
    ...
    for (const num of subArray) {
      if (typeof num === 'number' && isDivisibleByFive(num)) {
        newSubArray.push(powerOfTwo(num));
      }
    }
...
let findShortest = (data) => {
  const shortestCharacter = data
    .filter(character => character.height !== 'n/a') // Remove characters with unknown height
    .reduce((shortest, current) => {
      const currentHeight = parseInt(current.height);
      const shortestHeight = shortest ? parseInt(shortest.height) : Infinity;
