...
const sortByChildren = (charArray) => {
  const compareByChildren = (char1, char2) => {
    if (char1.children.length < char2.children.length) {
      return -1;
  ...
  return charArray.slice().sort(compareByChildren);
};
...
const citiesAtoJ = (arr) => {
  const pattern = /^[A-J]/i;
  const filteredCities = arr.filter((city) => pattern.test(city));
...
let hangman = (str) => {
  ...
  const result = str.replace(vowelRegex, "_");

  return result;
};
//Test the function
const inputString = "Welcome to Code 301!";
const outputString = hangman(inputString);
...
const findShells = (str) => {
  const pattern = /sells|seashells|shells/g;
  const matches = str.match(pattern);
  return matches || [];
};
